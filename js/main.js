...
client.getUserCoursesAndModules(function(error, data) {
    ...
    client.getCourseModules(data.courses[0].id, function (error, Modules) {
        if (modulesWrapper) {
            for (let i = 0; i < Modules.length; i++, j++) {
                if (j == 3) break;
                content += '<div class=\"planets__card\"\><div class=\"planets__img\"\><img src=\"./img/planet' + (j + 1) + '.png\" alt=\"planet image\"\></div\><h3>' +  Modules[i].title + '</h3\></div\>'
            }
            modulesWrapper.innerHTML = content;
        } else if (modulesMain) {
            for (let i = 0; i < Modules.length; i++, j++) { 
                content += '<div class=\"planets__card\"\><div class=\"planets__img\"\><img src=\"./img/planet' + (j < 9 ? (j + 1) : (j % 9 + 1)) + '.png\" alt=\"planet image\"\></div\><h3>' +  Modules[i].title + '</h3\></div\>' 
            }
        }
        if (modulesMain) modulesMain.innerHTML = content;
    });
    if (modulesWrapper) {
        for (let i = 0; i < data.modules.length; i++, j++) {
            if (j == 3) break;
            content += '<div class=\"planets__card\"\><div class=\"planets__img\"\><img src=\"./img/planet' + (j + 1) + '.png\" alt=\"planet image\"\></div\><h3>' +  data.modules[i].title + '</h3\></div\>'
        }
        modulesWrapper.innerHTML = content;
    } else if (modulesMain) {
        for (let i = 0; i < data.modules.length; i++, j++) { 
            content += '<div class=\"planets__card\"\><div class=\"planets__img\"\><img src=\"./img/planet' + (j < 9 ? (j + 1) : (j % 9 + 1)) + '.png\" alt=\"planet image\"\></div\><h3>' +  data.modules[i].title + '</h3\></div\>' 
        }
    }
});
